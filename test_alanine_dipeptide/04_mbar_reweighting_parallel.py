#!/usr/bin/env python
"""
步骤 4: REST2 HREMD 的 MBAR 重加权分析 【多进程并行版本】
...
print("="*70)
print(" REST2 HREMD 数据的 MBAR 重加权分析 【多进程并行版】")
print("="*70)
...
# 🚀 多进程配置
N_WORKERS = min(cpu_count() - 1, 8)  # 保留1个核心给系统
print(f"🚀 多进程配置: {N_WORKERS} 个工作进程 (CPU核心数: {cpu_count()})")
...
# =========================================================================
# 🚀 多进程 Worker 函数（必须在顶层定义）
# =========================================================================
def process_frame_worker(args):
    """
    在子进程中处理单个帧：读取DCD并计算二面角

    参数:
        args: (n, frame_info, weight, traj_dir_str, topology_file_str, traj_interval)

    返回:
        成功: (n, phi_deg, psi_deg, weight, None)
        失败: (n, None, None, None, error_message)
    """
...
# =========================================================================
# Part 6: 轨迹分析与重加权 【多进程并行版】
# =========================================================================
print("\n" + "="*70)
print("[Part 6/8] 轨迹分析：读取构象并计算二面角 【多进程并行】")
print("="*70)
...
with Pool(processes=N_WORKERS) as pool:
    ...
    for i, result in enumerate(pool.imap_unordered(process_frame_worker, args_list, chunksize=chunksize)):
        ...
        if (i + 1) % max(1, len(args_list) // 20) == 0:
            print(f"  进度: {i+1}/{len(args_list)} ({100*progress:.1f}%)")

t_end = time.time()
print(f"\n⏱️  多进程耗时: {t_end - t_start:.2f} 秒")
...
for n, phi, psi, weight, error in results:
    if error is None:
        ...
        if failed_count <= 3:  # 只打印前3个错误
            print(f"⚠️ 索引 {n} 失败: {error}")

if failed_count > 3:
    print(f"⚠️ 共 {failed_count} 个帧读取失败")
...
print(f"🚀 估算加速比: {speedup:.1f}x (相比串行)")
...
report_lines.append(" REST2 HREMD MBAR重加权分析报告【多进程并行版】")
report_lines.append("="*70)
report_lines.append("")
...
report_lines.append("【多进程性能】")
report_lines.append(f"  工作进程数: {N_WORKERS}")
report_lines.append(f"  Part 6 耗时: {t_end - t_start:.2f} 秒")
report_lines.append(f"  估算加速比: {speedup:.1f}x")
report_lines.append("")
